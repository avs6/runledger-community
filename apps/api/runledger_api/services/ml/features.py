...
async def _materialize_cost_features(
    ...
    row = result.one()
    return await _upsert_feature(db, workspace_id, target_date, "cost", None, {
...
async def _materialize_latency_features(
    ...
    if row.call_count == 0:
        return 0
    return await _upsert_feature(db, workspace_id, target_date, "latency", None, {
...
async def _materialize_error_features(
    ...
    return await _upsert_feature(db, workspace_id, target_date, "error_rate", None, {
        "value": error_rate,
...
async def _materialize_token_features(
    ...
    return await _upsert_feature(db, workspace_id, target_date, "tokens", None, {
        "value": total,
...
async def _materialize_cache_features(
    ...
    return await _upsert_feature(db, workspace_id, target_date, "cache_hit_rate", None, {
        "value": hit_rate,
        "total_calls": row.total,
...
async def _materialize_per_model_features(
    ...
    for row in result.all():
        ...
        count += await _upsert_feature(db, workspace_id, target_date, "cost_by_model", row.model, {
