...
async def forward_request(
    ...
    """
    Forward the completion request to the provider.

    Returns the raw provider JSON response dict.
    Raises httpx.HTTPStatusError on 4xx/5xx from the provider.
    """
    api_key = os.getenv(route.api_key_env_var or "OPENAI_API_KEY", "")
...
async def route_and_forward(
    ...
    temperature: float | None,
    max_tokens: int | None,
) -> tuple[dict[str, Any], GatewayRoute, int]:
    """
    Try each route in priority order, retrying on 429 / 5xx.
    Returns (response_json, winning_route, latency_ms).
    Raises HTTPException 502 when all routes fail.
    """
    routes = await select_routes(db, workspace_id, model_alias)
    if not routes:
        raise HTTPException(
    ...
    for route in routes:
        t0 = time.monotonic()
        try:
            response = await forward_request(route, messages, temperature, max_tokens)
            latency_ms = int((time.monotonic() - t0) * 1000)
            return response, route, latency_ms
        except httpx.HTTPStatusError as exc:
            last_error = exc
            if exc.response.status_code not in (429, 500, 502, 503, 504):
                # Client errors (4xx except 429) — don't try next route
                raise HTTPException(
                    status_code=status.HTTP_502_BAD_GATEWAY,
            ...
            last_error = exc
            log.warning("gateway_route_error", route_id=str(route.id), error=str(exc))
