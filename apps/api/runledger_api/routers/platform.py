...
async def _ws_detail(db: AsyncSession, ws: Workspace) -> dict[str, Any]:
    ...
    mc = (
        await db.execute(
            ...
            .where(WorkspaceUser.workspace_id == ws.id)
        )
    ).scalar() or 0
    return {
        "id": ws.id,
...
async def platform_update_org_status(
    ...
    if tenant.is_default and body.status != "active":
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "Cannot suspend or archive the default organization"
        )
...
async def platform_delete_org(
    ...
    member_ids = list(
        (await db.execute(select(TenantUser.user_id).where(TenantUser.tenant_id == org_id)))
        .scalars()
        .all()
    )
    exclusive_user_ids = []
...
async def platform_list_workspaces(
    ...
    if search:
        query = query.where(Workspace.name.ilike(f"%{search}%") | Tenant.name.ilike(f"%{search}%"))
...
async def platform_update_user(
    ...
    if body.username is not None:
        if (
            ...
            raise HTTPException(
                status.HTTP_409_CONFLICT, f"Username '{body.username}' already taken"
            )
...
async def platform_add_user_to_org(
    ...
    existing = (
        await db.execute(
            select(TenantUser).where(
                TenantUser.tenant_id == body.tenant_id, TenantUser.user_id == user_id
            )
        )
    ).scalar_one_or_none()
    if existing is not None:
    ...
    ws_role = (
        WorkspaceRoleEnum.workspace_admin
        if role == TenantRoleEnum.org_admin
        else WorkspaceRoleEnum.member
    )
    workspaces = list(
        (await db.execute(select(Workspace).where(Workspace.tenant_id == body.tenant_id)))
        .scalars()
        .all()
    )
    for ws in workspaces:
        existing_wu = (
            await db.execute(
                select(WorkspaceUser).where(
                    WorkspaceUser.workspace_id == ws.id, WorkspaceUser.user_id == user_id
                )
            )
        ).scalar_one_or_none()
...
async def platform_update_user_org_role(
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY, f"Invalid role: {body.role}"
        ) from None
    await db.commit()
    tenant = await db.get(Tenant, org_id)
    return {
...
async def platform_remove_user_from_org(
    ...
    await db.delete(tu)
    ws_ids = list(
        (await db.execute(select(Workspace.id).where(Workspace.tenant_id == org_id)))
        .scalars()
        .all()
    )
    for wu in (
