...
FEATURE_TAGS = [
    "customer-support", "code-review", "document-analysis",
    "data-extraction",  "summarization", "classification",
    "email-drafting",   "report-generation",
]
...
class Client:
    ...
    async def post(self, path: str, *, json: dict | None = None,
        ...
        if files:
            ...
            r = await self._http.post(path, headers=self._headers({"Content-Type": "application/json"}),
                                      content=__import__("json").dumps(json or {}), params=params)
    ...
    async def get(self, path: str, *, params: dict | None = None,
                  skip_errors: bool = False) -> dict | list:
        assert self._http
    ...
    async def put(self, path: str, *, json: dict | None = None,
                  skip_errors: bool = False) -> dict:
        assert self._http
        r = await self._http.put(path, headers=self._headers({"Content-Type": "application/json"}),
    ...
    async def patch(self, path: str, *, json: dict | None = None) -> dict:
        assert self._http
        r = await self._http.patch(path, headers=self._headers({"Content-Type": "application/json"}),
                                   content=__import__("json").dumps(json or {}))
        try:
            return r.json()
        except Exception:
...
async def create_orgs(admin: Client) -> list[dict]:
    ...
    demo_users = []
    all_demo_emails = [email for org in ORGS for email, _, _ in org["users"]]
    for org in ORGS:
        for email, full_name, role in org["users"]:
            r = await admin.post("/org/members/invite", json={
                "email": email,
                "full_name": full_name,
                "role": role,
    ...
    for org in ORGS:
        ...
        for ws_name in org["workspaces"]:
            ...
            ws_resp = await admin.post("/org/workspaces", json={"name": full_name},
                                       skip_errors=True)
            ...
            if not ws_id:
                ...
                match = next((w for w in ws_list
                              if isinstance(w, dict) and w.get("name") == full_name), {})
                ws_id = match.get("id")
            ...
            switch = await admin.post("/auth/switch-workspace",
                                      json={"workspace_id": ws_id}, skip_errors=True)
...
async def seed_workspace(ctx: dict) -> None:
    ...
    async with Client(BASE_URL, api_key) as c:
        ...
        for tool_name, desc, policy, enforcement in TOOL_DEFS:
            await c.post("/tools/registry", json={
                "tool_name": tool_name,
                "description": desc,
                "policy": policy,
        ...
        routes_created = []
        gateway_routes = [
            {"alias": "gpt4o-primary",     "provider": "openai",    "target_model": "gpt-4o",
             "priority": 10, "daily_cost_limit_usd": 500, "monthly_cost_limit_usd": 12000,
             "pii_redaction_enabled": True, "per_user_rpm_limit": 60, "health_auto_disable": True},
            {"alias": "gpt4o-mini-budget", "provider": "openai",    "target_model": "gpt-4o-mini",
             "priority": 20, "daily_cost_limit_usd": 150, "monthly_cost_limit_usd": 4000,
             "pii_redaction_enabled": False, "per_user_rpm_limit": 120, "health_auto_disable": True},
            {"alias": "claude-sonnet",     "provider": "anthropic", "target_model": "claude-sonnet-4-6",
             "priority": 15, "daily_cost_limit_usd": 300, "monthly_cost_limit_usd": 8000,
             "pii_redaction_enabled": True, "per_user_rpm_limit": 60, "health_auto_disable": True},
            {"alias": "gemini-flash",      "provider": "google",    "target_model": "gemini-2.0-flash",
             "priority": 25, "daily_cost_limit_usd": 100, "monthly_cost_limit_usd": 2500,
             "pii_redaction_enabled": False, "per_user_rpm_limit": 200, "health_auto_disable": True},
        ]
        for rd in gateway_routes:
            r = await c.post("/gateway/routes", json={**rd,
                "api_key_env_var": ("OPENAI_API_KEY" if rd["provider"] == "openai"
                                    else "ANTHROPIC_API_KEY" if rd["provider"] == "anthropic"
                                    else "GEMINI_API_KEY"),
            }, skip_errors=True)
            if "id" in r:
        ...
        # Routing policies
        routing_policies = [
            {"alias": "cost-optimized",   "policy_type": "cost_optimized",
             "config": {"max_latency_ms": 5000}},
            {"alias": "quality-first",    "policy_type": "quality_optimized",
             "config": {"min_avg_score": 0.80}},
            {"alias": "budget-aware",     "policy_type": "budget_aware",
             "config": {"daily_budget_usd": 200, "fallback_model": "gpt-4o-mini"}},
            {"alias": "canary-claude",    "policy_type": "canary",
             "config": {"canary_weight": 0.1, "canary_alias": "claude-sonnet"}},
            {"alias": "weighted-blend",   "policy_type": "weighted",
        ...
        for day in range(30):
            ...
            for _ in range(daily_runs):
                ...
                status = random.choices(
                    ["succeeded", "failed", "cancelled"], weights=[85, 10, 5]
                )[0]
                ...
                events = [
                    {"event_type": "run_start", "run_id": run_id,
                     "end_user_id": end_user, "session_id": f"sess_{uuid.uuid4().hex[:10]}",
                     "feature_tag": feature, "deployment_version": version,
                     "started_at": started, "metadata": {"sdk": "runledger-sdk@2.1.0"}},
                    {"event_type": "span_start", "span_id": span_id, "run_id": run_id,
                     "span_type": "llm", "name": f"{provider}.chat",
                     "started_at": started},
                ...
                if random.random() < 0.5:
                    events.append({
                        "event_type": "tool_call", "run_id": run_id, "span_id": span_id,
                        "tool_name": tool_name, "tool_type": "read",
                ...
                events += [
                    {"event_type": "span_end", "span_id": span_id, "run_id": run_id,
                     ...
                     "ended_at": ended, "cost_usd": cost},
                    {"event_type": "run_end", "run_id": run_id,
                ...
                run_meta.append({
                    "run_id": run_id, "end_user": end_user,
                    "feature": feature, "provider": provider, "model": model,
                    "cost": cost, "status": status, "day": day,
                })
        ...
        for meta in random.sample(run_meta, min(len(run_meta), 40)):
            ...
            await c.post("/evaluations/scores", json={
                "run_id": meta["run_id"],
                "name": name,
                "value": val,
        ...
        evaluators = [
            {"name": "helpfulness-judge",
             ...
             "config": {"model": "gpt-4o", "score_name": "helpfulness",
            ...
            {"name": "safety-classifier",
             ...
             "evaluator_type": "llm_judge",
             "config": {"model": "gpt-4o-mini", "score_name": "safety",
            ...
            {"name": "accuracy-scorer",
             ...
             "evaluator_type": "llm_judge",
             "config": {"model": "gpt-4o", "score_name": "accuracy",
        ...
        for name, desc, content, variables in PROMPT_DEFS:
            ...
            # Add 3 versions
            for v in range(1, 4):
                suffix = "" if v == 1 else " [optimised for token efficiency]" if v == 2 else " [concise variant]"
                await c.post(f"/prompts/{name}/versions", json={
                    "content": content + suffix,
                    "variables": variables,
                    "commit_message": (
                        "Initial version" if v == 1
                        else "Reduced token count by 30%" if v == 2
        ...
        dataset_items_customer_qa = [
            {"input": "What is your refund policy?",                    "expected_output": "We offer 30-day refunds.",  "metadata": {}},
            {"input": "How do I reset my password?",                     "expected_output": "Click 'Forgot password'.",  "metadata": {}},
            {"input": "Can I upgrade my plan mid-month?",               "expected_output": "Yes, prorated billing.",    "metadata": {}},
            {"input": "Do you support bulk exports?",                   "expected_output": "Yes, via the API.",         "metadata": {}},
            {"input": "What models does RunLedger support?",            "expected_output": "OpenAI, Anthropic, Google, Mistral.", "metadata": {}},
            {"input": "How is cost calculated?",                        "expected_output": "Per 1M tokens × rate.",     "metadata": {}},
            {"input": "Is there a free tier?",                          "expected_output": "Yes, 100k tokens/month.",   "metadata": {}},
            {"input": "How do I set a spending budget?",                "expected_output": "Under Budgets in the dashboard.", "metadata": {}},
        ]
        dataset_items_code_review = [
            {"input": "Review: def add(a,b): return a+b",              "expected_output": "Looks correct.",            "metadata": {}},
            {"input": "Review: for i in range(len(lst)):",             "expected_output": "Use enumerate() instead.",  "metadata": {}},
            {"input": "Review: x = None; print(x.upper())",            "expected_output": "NoneType error risk.",      "metadata": {}},
            {"input": "Review: SELECT * FROM users",                   "expected_output": "Specify columns, not *.",   "metadata": {}},
            {"input": "Review: import *",                               "expected_output": "Avoid wildcard imports.",  "metadata": {}},
        ]
        datasets_created = []
        for ds_name, ds_desc, ds_items in [
            ("Customer Support Q&A",   "Common customer support questions with expected answers",   dataset_items_customer_qa),
            ("Code Review Samples",    "Code snippets for automated review evaluation",             dataset_items_code_review),
        ]:
            r = await c.post("/datasets", json={
                "name": ds_name, "description": ds_desc, "source": "manual", "items": ds_items,
            }, skip_errors=True)
            if "id" in r:
            ...
            experiment_defs = [
                {
                    ...
                    "models": [
                        ...
                        {"model": "claude-sonnet-4-6", "provider": "anthropic", "label": "Claude Sonnet"},
                    ],
                },
                {
        ...
        approval_defs = [
            {"request_type": "budget_increase",
             "request": {"scope": "workspace", "current_limit_usd": 10000, "requested_limit_usd": 18000}},
            {"request_type": "gateway_route_add",
             "request": {"alias": "gpt-4-turbo", "provider": "openai", "target_model": "gpt-4-turbo"}},
        ...
        if prompt_names and approval_ids:
            ...
            await c.post(f"/prompts/{prompt_names[0]}/promote",
                         json={"target": "production", "version": 3},
                         params={"approval_id": ap_id}, skip_errors=True)
        ...
        notif = await c.post("/budgets/notifications", json={
            "channel": "webhook",
        ...
        budget_defs = [
            {"scope_type": "workspace",   "scope_id": None,
             "period_type": "monthly",    "limit_usd": 12000, "action": "notify"},
            {"scope_type": "workspace",   "scope_id": None,
             "period_type": "daily",      "limit_usd": 600,   "action": "notify"},
            {"scope_type": "feature_tag", "scope_id": "customer-support",
             "period_type": "daily",      "limit_usd": 400,   "action": "block"},
            {"scope_type": "feature_tag", "scope_id": "code-review",
             "period_type": "daily",      "limit_usd": 250,   "action": "downgrade",
             "downgrade_to_model": "gpt-4o-mini"},
            {"scope_type": "end_user",    "scope_id": "user_001",
             "period_type": "daily",      "limit_usd": 25,    "action": "block"},
            {"scope_type": "end_user",    "scope_id": "user_002",
             "period_type": "monthly",    "limit_usd": 200,   "action": "notify"},
        ]
        for bd in budget_defs:
            await c.post("/budgets", json={
        ...
        await asyncio.sleep(_THROTTLE)
        await c.get("/budgets")
        await c.get("/budgets/check", params={
            "scope_type": "workspace", "period_type": "daily"
        }, skip_errors=True)
        ...
        alert_defs = [
            ...
            {"name": "P95 Latency Spike",  "metric": "p95_latency",    "operator": "gt",
             "threshold": 5000,  "window_minutes": 60},
            {"name": "Low Quality Score",  "metric": "avg_score",      "operator": "lt",
             "threshold": 0.70,  "window_minutes": 60},
            {"name": "Spend Velocity",     "metric": "spend_velocity", "operator": "gt",
             "threshold": 50.00, "window_minutes": 60},
        ]
        alert_ids = []
        for ad in alert_defs:
            r = await c.post("/alerts/rules", json={
                "name": ad["name"], "metric": ad["metric"],
                "operator": ad["operator"], "threshold": ad["threshold"],
                "window_minutes": ad["window_minutes"],
                "action": "notify", "is_active": True, "email_enabled": True,
            }, skip_errors=True)
            if "id" in r:
        ...
        # Cost centres
        cc_resp = await c.post("/billing/cost-centers", json={
            "name": "Engineering", "code": "ENG",
            "description": "Core engineering and platform teams",
        }, skip_errors=True)
        cc_eng_id = cc_resp.get("id")

        cc_prod = await c.post("/billing/cost-centers", json={
        ...
        cc_cx = await c.post("/billing/cost-centers", json={
        ...
        # Chargeback rules
        for rule in [
            {"allocation_type": "team", "dimension": "engineering", "weight": 0.50, "cost_center_id": cc_eng_id},
            {"allocation_type": "team", "dimension": "product",     "weight": 0.30, "cost_center_id": cc_prod_id},
            {"allocation_type": "team", "dimension": "cx",          "weight": 0.20, "cost_center_id": cc_cx_id},
        ]:
        ...
        # Shared cost policy
        await c.post("/billing/shared-cost-policies", json={
        ...
        for months_back in range(3, 0, -1):
            ...
            period = await c.post("/billing/periods", json={
            ...
            # Add an adjustment
            await c.post(f"/billing/periods/{pid}/adjustments", json={
            ...
            await c.get(f"/billing/periods/{pid}/export", params={"format": "csv"}, skip_errors=True)

        # Current open period
        open_period = await c.post("/billing/periods", json={
        ...
        bw = await c.post("/billing/webhooks", json={
        ...
        for months_back in range(1, 3):
            ...
            for provider, model, in_r, out_r in random.sample(PROVIDER_MODELS, 2):
                ...
                writer.writerow(["request_id", "model", "input_tokens",
                ...
                for i in range(random.randint(15, 30)):
                    ...
                    occurred = p_start + timedelta(days=random.randint(0, 25),
                                                    hours=random.randint(0, 23))
                    writer.writerow([
                        f"req_{uuid.uuid4().hex[:24]}", model,
                        in_t, out_t, round(cost_line, 6),
                        occurred.isoformat(),
                    ])
                ...
                inv = await c.post("/invoices/upload", files=files, params={
                    "period_start": p_start.isoformat(),
                    "period_end":   p_end.isoformat(),
                    "provider":     provider,
                }, skip_errors=True)
                ...
                if inv_id:
                    ...
                    await c.get(f"/invoices/{inv_id}/export", params={"format": "json"}, skip_errors=True)
        ...
        print("    → Outcomes & ROI")
        outcome_types = [
            "task_completed", "user_satisfied", "ticket_resolved",
            "revenue_generated", "error_handled",
        ]
        ...
        # Annotations
        await c.post("/analytics/annotations", json={
        ...
        ret_defs = [
            {"resource_type": "spans",          "action": "delete",  "scope": "workspace",
             "max_age_days": 365},
            {"resource_type": "provider_calls",  "action": "scrub",   "scope": "workspace",
             "max_age_days": 180},
            {"resource_type": "agent_runs",      "action": "delete",  "scope": "end_user",
             "scope_value": "user_001", "max_age_days": 90},
        ]
        for rd in ret_defs:
        ...
        if isinstance(ret_items, list) and ret_items:
            ...
            if first_id:
                await c.post("/retention/purge", json={
                    "policy_id": first_id, "dry_run": True
                }, skip_errors=True)
        ...
        contract = await c.post("/pricing/contracts", json={
            "provider": "openai",
            "model": "gpt-4o",
            "discount_pct": 15.0,
        ...
        await c.post("/pricing/contracts", json={
            "provider": "anthropic",
            "model": None,
            "discount_pct": 10.0,
        ...
        await c.post("/pricing/credits", json={
        ...
        await c.post("/warehouse/destinations", json={
            "name": "S3 Data Lake",
            "destination_type": "s3",
            "config": {
...
async def main() -> None:
    ...
    async with Client(BASE_URL) as root:
        ...
        auth = await root.post("/auth/login", json={
