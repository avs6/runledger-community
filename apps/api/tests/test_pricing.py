...
# ── pricing_sync: load_pricing_yaml ───────────────────────────────────────────


def test_load_pricing_yaml_basic(tmp_path: object) -> None:
    """Parses a minimal YAML file and returns correct PricingEntry objects."""
    import pathlib

    f = pathlib.Path(str(tmp_path)) / "pricing.yml"  # type: ignore[arg-type]
    f.write_text(
        """
models:
  - provider: openai
...
""",
    ...
    from runledger_api.services.pricing_sync import load_pricing_yaml

    entries = load_pricing_yaml(str(f))
    assert len(entries) == 2
    assert entries[0].provider == "openai"
    assert entries[0].model == "gpt-4o"
    assert entries[0].input_per_1m == Decimal("2.50")
    assert entries[0].cached_input_per_1m == Decimal("1.25")
    assert entries[1].provider == "ollama"
    assert entries[1].cached_input_per_1m is None


def test_load_pricing_yaml_missing_file() -> None:
    """Returns empty list without raising if the file does not exist."""
    from runledger_api.services.pricing_sync import load_pricing_yaml

    entries = load_pricing_yaml("/nonexistent/pricing.yml")
    assert entries == []


@pytest.mark.asyncio
async def test_sync_pricing_inserts_new_row() -> None:
    """Inserts a row when no existing global row matches the provider+model."""
    from runledger_api.services.pricing_sync import PricingEntry, sync_pricing
    ...
    entry = PricingEntry(
        provider="ollama",
        model="llama3.2",
        input_per_1m=Decimal("0.00"),
        output_per_1m=Decimal("0.00"),
    )
    counts = await sync_pricing(session, [entry])

    assert counts == {"inserted": 1, "updated": 0, "unchanged": 1 - 1}
    session.add.assert_called_once()


@pytest.mark.asyncio
async def test_sync_pricing_unchanged_is_noop() -> None:
    """No DB write when the existing row already has the same prices."""
    from runledger_api.services.pricing_sync import PricingEntry, sync_pricing
...
@pytest.mark.asyncio
async def test_sync_pricing_updates_on_price_change() -> None:
    """Closes the old row and inserts a new one when the output price changes."""
    from runledger_api.services.pricing_sync import PricingEntry, sync_pricing
    ...
    entry = PricingEntry(
        ...
        model="gpt-4o",
        input_per_1m=Decimal("2.50"),
        output_per_1m=Decimal("12.00"),  # price increase
    )
    counts = await sync_pricing(session, [entry])

    assert counts == {"inserted": 0, "updated": 1, "unchanged": 0}
    # SELECT + UPDATE both go through session.execute
    assert session.execute.call_count == 2
    session.add.assert_called_once()  # new effective row inserted
