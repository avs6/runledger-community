...
async def test_signup_creates_account(client: AsyncClient, mock_db_session: AsyncMock) -> None:
    ...
    with patch(
        "runledger_api.routers.saas.generate_api_key",
        return_value=("rl_test_abc", "hash", "rl_test_"),
    ):
...
@pytest.mark.asyncio
async def test_get_subscription_returns_plan(
...
@pytest.mark.asyncio
async def test_get_subscription_no_rows_returns_free_defaults(
...
@pytest.mark.asyncio
async def test_quota_check_blocks_at_limit(
    authed_client: AsyncClient, mock_db_session: AsyncMock, mock_workspace: SimpleNamespace
) -> None:
...
@pytest.mark.asyncio
async def test_quota_check_allows_when_under_limit(
    authed_client: AsyncClient, mock_db_session: AsyncMock, mock_workspace: SimpleNamespace
) -> None:
...
@pytest.mark.asyncio
async def test_quota_check_allows_when_no_row(
    authed_client: AsyncClient, mock_db_session: AsyncMock
) -> None:
...
async def test_stripe_webhook_without_stripe_returns_503(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        "/webhooks/stripe", content=b"{}", headers={"stripe-signature": "t=1,v1=abc"}
    )
    assert resp.status_code == 503
