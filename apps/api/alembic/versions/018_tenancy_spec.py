...
def upgrade() -> None:
    ...
    conn.execute(
        sa.text("ALTER TYPE workspace_role_enum ADD VALUE IF NOT EXISTS 'workspace_editor'")
    )
    conn.execute(
        sa.text("ALTER TYPE workspace_role_enum ADD VALUE IF NOT EXISTS 'workspace_contributor'")
    )
    conn.execute(sa.text("BEGIN"))

    # ── tenants: status ────────────────────────────────────────────────────────
    op.add_column(
        "tenants", sa.Column("status", sa.String(16), nullable=False, server_default="active")
    )

    # ── workspaces: status + is_restricted ─────────────────────────────────────
    op.add_column(
        "workspaces", sa.Column("status", sa.String(16), nullable=False, server_default="active")
    )
    op.add_column(
        "workspaces",
    ...
    # ── tenant_users: status ───────────────────────────────────────────────────
    op.add_column(
        "tenant_users", sa.Column("status", sa.String(16), nullable=False, server_default="active")
    )
