...
function FeatureChart({ data }: { data: SpendByFeature }) {
  ...
  const bars = data.items
    ...
    .map(f => ({ name: f.feature_tag!, cost: parseFloat(f.cost_usd) }))
  return (
    <div className="space-y-4">
      <ResponsiveContainer width="100%" height={200}>
...
export default function AnalyticsBreakdownClient() {
  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <Card title="Spend Over Time" sub={presetSubtitle(preset, presetGranularity(preset))} action={<ArrowUpRight className="h-4 w-4 text-slate-300 dark:text-slate-600" />}>
      ...
      <Card title="Spend by End User" sub="Attribution per end_user_id — top 20" action={<Users className="h-4 w-4 text-slate-300 dark:text-slate-600" />}>
