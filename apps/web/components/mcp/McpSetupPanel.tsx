...
const inputCls =
  'rounded-xl border border-slate-300 bg-white/80 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 shadow-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-200 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-blue-500 dark:focus:ring-blue-500/20'
  ...
  if (!canManageOrgSettings) {
    return (
      <div className="max-w-3xl p-8">
        <h1 className="text-2xl font-semibold text-slate-950 dark:text-white">MCP</h1>
    ...
    <div className="space-y-8">
      {showPageTitle ? (
        <div className="max-w-5xl">
          <h1 className="text-3xl font-semibold text-slate-950 dark:text-white">MCP</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-400">
            Connect IDEs, desktop agents, hosted agents, model gateways, and workflow tools to RunLedger.
            Use RunLedger inline for budget, policy, and routing decisions, and out of band for observability,
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-start gap-3">
            <div className="rounded-2xl bg-blue-100 p-2 text-blue-700 dark:bg-blue-950/60 dark:text-blue-400">
              <Key className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Setup Wizard</h2>
              ...
              return (
                <button
                  ...
                  className={`rounded-2xl border p-4 text-left transition ${
                    isSelected
                      ? 'border-blue-300 bg-blue-50 shadow-sm ring-2 ring-blue-100 dark:border-blue-700 dark:bg-blue-950/40 dark:ring-blue-900/40'
                      : 'border-slate-200 bg-slate-50/70 hover:border-blue-200 hover:bg-white dark:border-slate-700 dark:bg-slate-800/60 dark:hover:border-blue-700 dark:hover:bg-slate-800'
                  }`}
                >
                  <div className="flex items-center justify-between gap-3">
                    <Icon className={`h-5 w-5 ${isSelected ? 'text-blue-700 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'}`} />
                    <span className="rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold text-slate-500 dark:bg-slate-700 dark:text-slate-300">
                      {connector.label}
                    </span>
                  </div>
                  <p className="mt-3 text-sm font-semibold text-slate-950 dark:text-white">{connector.name}</p>
                  <p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-400">{connector.description}</p>
                </button>
              )
            })}
          </div>
          ...
          <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
            ...
            <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
              Current workspace: <span className="font-semibold text-slate-700 dark:text-slate-200">{workspaceName ?? 'selected workspace'}</span>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-slate-950 dark:text-white">{selected.name}</h2>
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{selected.configTitle}</p>
            </div>
            <button
              type="button"
              onClick={() => copy(snippet, selected.id)}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-blue-300 hover:text-blue-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-blue-600 dark:hover:text-blue-400"
            >
          ...
          <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-xs leading-5 text-slate-700 dark:border-blue-900/60 dark:bg-blue-950/40 dark:text-slate-300">
            <p className="font-semibold text-slate-950 dark:text-white">Smoke test</p>
            <p className="mt-1">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-950 dark:text-white">MCP Tools</h2>
          </div>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Inline control and out-of-band telemetry.</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {mcpTools.map((item) => (
              <code key={item} className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                {item}
              </code>
            ))}
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center gap-2">
            <Server className="h-5 w-5 text-blue-700 dark:text-blue-400" />
            <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Resources</h2>
          </div>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Read-only context agents can fetch when diagnosing work.</p>
          <div className="mt-4 space-y-2">
            {resources.map((item) => (
              <code key={item} className="block rounded-xl bg-slate-100 px-3 py-2 text-[11px] text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                {item}
              </code>
            ))}
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Prompts</h2>
          </div>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Reusable agent workflows for consistent RunLedger behavior.</p>
          <div className="mt-4 space-y-2">
            {prompts.map((item) => (
              <code key={item} className="block rounded-xl bg-slate-100 px-3 py-2 text-[11px] text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                {item}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
        <div className="flex items-center gap-2">
          <RadioTower className="h-5 w-5 text-blue-700 dark:text-blue-400" />
          <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Connection Modes</h2>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {[
            ...
            <div key={title} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
              <p className="text-sm font-semibold text-slate-950 dark:text-white">{title}</p>
              <code className="mt-2 block rounded-lg bg-white px-2 py-1 text-[11px] text-slate-600 dark:bg-slate-900 dark:text-slate-300">{value}</code>
