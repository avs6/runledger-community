...
function roleBadge(role: string | null, isPlatformAdmin: boolean) {
  if (isPlatformAdmin) return { label: 'Platform Admin', cls: 'bg-teal-100 text-teal-800 dark:bg-teal-500/15 dark:text-teal-200' }
  if (role === 'org_admin') return { label: 'Org Admin', cls: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-500/15 dark:text-cyan-200' }
  if (role === 'workspace_admin') return { label: 'WS Admin', cls: 'bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300' }
  if (role === 'member') return { label: 'Member', cls: 'bg-slate-100 text-slate-600 dark:bg-slate-800/60 dark:text-slate-300' }
  if (role === 'viewer') return { label: 'Viewer', cls: 'bg-slate-100 text-slate-500 dark:bg-slate-800/40 dark:text-slate-400' }
  return null
}
    ...
    >
          ...
          return (
            ...
            >
              <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold text-white ${
                isCurrent ? 'bg-teal-700 dark:bg-teal-500' : 'bg-gradient-to-br from-slate-400 to-slate-500'
              }`}>
                {ws.name.charAt(0).toUpperCase()}
              </div>
              <span className="flex-1 text-left font-medium truncate">{ws.name}</span>
              {isCurrent && <Check className="h-4 w-4 text-teal-600 dark:text-teal-300 shrink-0" />}
            </button>
          )
        })
      )}
      <div className="mx-3 border-t border-slate-100 dark:border-slate-800 mt-1 pt-1">
        {(isOrgAdmin || isPlatformAdmin) && (
          <a href="/workspace" className="flex w-full items-center gap-2 px-0 py-1.5 text-xs text-slate-500 hover:text-teal-700 dark:text-slate-400 dark:hover:text-teal-300 transition-colors">
  ...
  return (
    <header className="relative z-30 flex h-14 items-center justify-between border-b border-slate-200/80 bg-white/95 px-4 dark:border-slate-700/70 dark:bg-slate-900/95">
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          <LayoutGrid className={`h-3.5 w-3.5 ${switching ? 'animate-spin text-teal-500' : 'text-teal-700 dark:text-teal-300'}`} />
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          ...
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-teal-100 dark:bg-teal-900/30">
              {isPlatformAdmin
                ? <Shield className="h-3.5 w-3.5 text-teal-700 dark:text-teal-300" />
