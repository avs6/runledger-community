...
function roleBadge(role: string | null, isPlatformAdmin: boolean) {
  if (isPlatformAdmin) return { label: 'Platform Admin', cls: 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' }
  if (role === 'org_admin') return { label: 'Org Admin', cls: 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300' }
  if (role === 'workspace_admin') return { label: 'WS Admin', cls: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' }
  if (role === 'member') return { label: 'Member', cls: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300' }
  if (role === 'viewer') return { label: 'Viewer', cls: 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300' }
  return null
}
...
export default function TopBar() {
  ...
  const wsDropdown = mounted && wsMenuOpen && dropdownPos ? createPortal(
    <div
      ref={dropdownRef}
      className="fixed w-64 rounded-xl border border-slate-200 bg-white py-1.5 shadow-xl dark:border-slate-700 dark:bg-slate-900"
          ...
          return (
            ...
            >
              <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold text-white ${
                isCurrent ? 'bg-teal-500' : 'bg-gradient-to-br from-slate-400 to-slate-500'
              }`}>
                {ws.name.charAt(0).toUpperCase()}
              </div>
              <span className="flex-1 text-left font-medium truncate">{ws.name}</span>
              {isCurrent && <Check className="h-4 w-4 text-teal-500 shrink-0" />}
            </button>
          )
        })
      )}
      <div className="mx-3 border-t border-slate-100 dark:border-slate-800 mt-1 pt-1">
        <a href="/workspace" className="flex w-full items-center gap-2 px-0 py-1.5 text-xs text-slate-500 hover:text-teal-600 dark:text-slate-400 dark:hover:text-teal-400 transition-colors">
  ...
  return (
    <header className="relative z-30 flex h-14 items-center justify-between border-b border-slate-200/80 bg-white/95 px-4 dark:border-slate-700/70 dark:bg-slate-900/95">
      ...
      <div className="flex items-center gap-1.5">
        {orgName && (
          <>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">{orgName}</span>
            </div>
        ...
        <button
          ...
          disabled={switching}
          className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-60"
        >
          <LayoutGrid className={`h-3.5 w-3.5 ${switching ? 'animate-spin text-teal-500' : 'text-teal-600 dark:text-teal-400'}`} />
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors"
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-teal-100 dark:bg-teal-900/50">
              {isPlatformAdmin
                ? <Shield className="h-3.5 w-3.5 text-violet-600 dark:text-violet-300" />
          ...
          {userMenuOpen && (
            <>
              ...
              <div className="absolute right-0 top-full z-50 mt-1 w-56 rounded-xl border border-slate-200 bg-white py-1.5 shadow-lg dark:border-slate-700 dark:bg-slate-900">
