...
function roleBadge(role: string | null, isPlatformAdmin: boolean) {
  if (isPlatformAdmin) return { label: 'Platform Admin', cls: 'bg-violet-100 text-violet-700 dark:bg-violet-500/15 dark:text-violet-300' }
  if (role === 'org_admin') return { label: 'Org Admin', cls: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300' }
          ...
          return (
            ...
            >
              <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold text-white ${
                isCurrent ? 'bg-violet-500' : 'bg-gradient-to-br from-slate-400 to-slate-500'
              }`}>
                {ws.name.charAt(0).toUpperCase()}
              </div>
              <span className="flex-1 text-left font-medium truncate">{ws.name}</span>
              {isCurrent && <Check className="h-4 w-4 text-violet-500 shrink-0" />}
            </button>
          )
        })
      )}
  ...
  return (
    <header className="relative z-30 flex h-14 items-center justify-between border-b border-slate-200/80 bg-white/95 px-4 dark:border-slate-700/70 dark:bg-slate-900/95">
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          <LayoutGrid className={`h-3.5 w-3.5 ${switching ? 'animate-spin text-violet-500' : 'text-violet-600 dark:text-violet-400'}`} />
          ...
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-violet-100 dark:bg-violet-900/30">
              {isPlatformAdmin
                ...
                : <User className="h-3.5 w-3.5 text-violet-700 dark:text-violet-300" />
              }
            </div>
