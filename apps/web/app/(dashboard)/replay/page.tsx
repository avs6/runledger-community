  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold dark:text-white">Replay</h1>
        <span className="text-sm text-gray-400 dark:text-gray-500">— cost-projection experiments</span>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="py-8 text-center">
                  <FlaskConical className="mx-auto mb-2 h-8 w-8 text-gray-300 dark:text-gray-600" />
                  <p className="text-sm text-gray-400 dark:text-gray-500">No datasets yet. Create one to start replaying.</p>
                </div>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                  ...
                  <tbody>
                    {datasets.map((d) => (
                      <tr key={d.id} className="border-b dark:border-gray-700 last:border-0">
                        <td className="py-1.5 pr-3 font-medium dark:text-gray-200">{d.name}</td>
                        <td className="py-1.5 pr-3 text-gray-500 dark:text-gray-400">{d.source}</td>
                        <td className="py-1.5 pr-3 dark:text-gray-300">{d.run_count}</td>
                        <td className="py-1.5 text-gray-400 dark:text-gray-500 text-xs">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-gray-700 dark:text-gray-300">New Dataset</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreateDataset} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="mt-1 w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 dark:text-gray-100 px-2 py-1 text-sm"
                    placeholder="my-dataset"
                  />
                </div>
                <div>
                  <label className="text-xs text-gray-500 dark:text-gray-400">Run IDs (one per line)</label>
                  <textarea
                    value={dsRunIds}
                    onChange={(e) => setDsRunIds(e.target.value)}
                    rows={4}
                    className="mt-1 w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 dark:text-gray-100 px-2 py-1 font-mono text-xs"
                    placeholder="uuid&#10;uuid&#10;…"
                  />
                </div>
                <div>
                ...
                <button
                  type="submit"
                  disabled={dsCreating}
                  className="rounded bg-gray-900 dark:bg-gray-100 dark:text-gray-900 px-3 py-1.5 text-sm text-white disabled:opacity-50"
                >
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="py-8 text-center">
                  <FlaskConical className="mx-auto mb-2 h-8 w-8 text-gray-300 dark:text-gray-600" />
                  <p className="text-sm text-gray-400 dark:text-gray-500">No experiments yet. Select a dataset and create one.</p>
                </div>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                  ...
                  <tbody>
                    {experiments.map((e) => (
                      <tr key={e.id} className="border-b dark:border-gray-700 last:border-0">
                        <td className="py-1.5 pr-3 font-medium dark:text-gray-200">{e.name}</td>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-gray-700 dark:text-gray-300">New Experiment</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreateExperiment} className="space-y-3">
                <div>
                  <label className="text-xs text-gray-500 dark:text-gray-400">Dataset</label>
                  <select
                    value={expDatasetId}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-gray-500 dark:text-gray-400">Model A</label>
                    <input
                      value={expModelA}
                  ...
                  <div>
                    <label className="text-xs text-gray-500 dark:text-gray-400">Model B</label>
                    <input
                      value={expModelB}
