...
| Platform / Utility | Platform settings | `STRONG` | `STRONG` | `PARTIAL` | `STRONG` | `STRONG` | `STRONG` | `N/A` | `N/A` | WU-002 adds platform-settings-convergence-posture endpoint with telemetry context (OTLP batches, spans, capture policies) and audit context (audit events, security events). Settings page now shows Telemetry & Capture Convergence card (cyan) and Audit & Compliance Convergence card (amber). Telemetry P→S, Audit log P→S. Prior: GET /analytics/gateway-internal-posture returns provider count, guardrails, cache, and throttle context with platform visibility. Model gateway, provider profiles, and guardrails are now fully platform-visible (STRONG). Retention, compliance, and backup are governance pack components (STRONG). |
