...
| Platform / Utility | Platform settings | `STRONG` | `STRONG` | `STRONG` | `STRONG` | `STRONG` | `STRONG` | `N/A` | `N/A` | WU-004 strengthens Monitoring: platform-admin-observe-posture endpoint provides monitoring context (alert rules, firings, guardrail events) visible from Organizations with drill-through, establishing platform-settings as part of the shared admin observe workflow. Monitoring P→S. Prior WU-002 adds convergence posture endpoint with telemetry and audit context. Telemetry P→S, Audit log P→S. Prior: gateway-internal-posture returns provider count, guardrails, cache, and throttle context. |
| Platform / Utility | Plugins | `N/A` | `N/A` | `N/A` | `N/A` | `N/A` | `N/A` | `STRONG` | `N/A` | WU-003 documents collapsed ownership model: Onboarding owns discovery, Evaluation studio consumes plugin connections as tool integrations in builder workflows. Postman now covers full plugin CRUD (7 requests), example 159 exercises the complete lifecycle. Evaluation studio P→S. Prior: Collapsed redirect with backend plugin governance consumed by MCP tool calls. |
