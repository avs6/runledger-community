...
| Feature | Route | Backend | UI | Actions | Docs | Postman | Scripts/Examples | Complete | Cohesion | Merge / Collapse | Fix Order | Fix Status | Delivery Audit Crosswalk | Notes |
|---------|-------|---------|----|---------|------|---------|------------------|----------|----------|------------------|-----------|------------|--------------------|-------|
| Budget tiers | `/budget-tiers` | `OK` | `LEGACY` | `OK` | `OK` | `OK` | `OK` | `OK` | `OK` | Collapse into `Model gateway` quota controls and keep only as a compatibility redirect. | P5 | `COMPLETE` | `6.5` | Bundle context: supports Bundle A indirectly but should remain collapsed under Gateway rather than reopened as a standalone FinOps product. Completed as a deliberate collapse on Friday, August 14, 2026. The standalone `/budget-tiers` page now exists only as a compatibility redirect into the Gateway quota-tiers section. Backend CRUD, tier assignment, and compatibility API coverage remain intact, but the primary UI ownership has moved to `/gateway#gateway-quota-tiers`. |
| Budgets | `/budgets` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `OK` | `OK` | `OK` | `NO` | `OK` | Keep separate as the parent FinOps control. | P5 | `NOT STARTED` | `6.4` | Bundle A - Budget Control Plane. Real runtime enforcement exists and UI can create/list/delete, but there is no update endpoint or edit flow; `/budgets/{id}` is breach history, not true budget detail/edit. |
| Budget detail | `/budgets/{id}` | `PARTIAL` | `PARTIAL` | `MISSING` | `PARTIAL` | `PARTIAL` | `OK` | `NO` | `PARTIAL` | Collapse into `Budgets` as breach/history detail, not a separate feature. | P5 | `NOT STARTED` | `6.4` | Bundle A - Budget Control Plane. Shipped route is effectively breach history only. Backend exposes `GET /budgets/{id}/breaches`, not a budget get/update surface, so this is not a full detail page. |
| Budget overrides | `/budget-overrides` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `OK` | `OK` | `NO` | `OK` | Collapse into `Budgets` as advanced override workflow. | P5 | `NOT STARTED` | `6.6` | Bundle A - Budget Control Plane. Override lifecycle is create/list/revoke with auto-expiry, not full CRUD. Useful and runtime-aligned, but no edit/detail/delete flow and no approval-specific UX despite docs/scripts hinting at that need. |
| Model budgets | `/model-budgets` | `OK` | `LEGACY` | `PARTIAL` | `PARTIAL` | `OK` | `OK` | `NO` | `OK` | Collapse into `Model gateway` or `API keys` advanced quota settings and keep only as a compatibility redirect. | P5 | `COMPLETE` | `6.7` | Bundle context: supports Bundle A indirectly but should stay collapsed into Gateway or API key quota controls. Completed as a deliberate collapse on Friday, August 14, 2026. The standalone `/model-budgets` page now exists only as a compatibility redirect into the Gateway model-quotas section. Backend lifecycle now includes create/list/update/delete, and the primary UI ownership has moved to `/gateway#gateway-model-quotas` so operators no longer need to paste API-key UUIDs into a separate page. |
| Billing periods | `/billing` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `OK` | `PARTIAL` | `NO` | `OK` | Keep separate as the parent billing area. | P5 | `NOT STARTED` | `6.9` | Bundle B - Billing and Reconciliation. Backend supports period create/list/get/close/export plus adjustments/shared-cost policies, but the page is mostly list/close/export and does not surface the fuller management model. |
| Billing period detail | `/billing/{period_id}` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `OK` | `PARTIAL` | `NO` | `OK` | Keep as billing detail. | P5 | `NOT STARTED` | `6.9` | Bundle B - Billing and Reconciliation. Good read path for reconciliation and breakdown, but billing adjustments exist only in the backend/Postman and are not manageable from the UI detail page. |
| Chargeback | `/chargeback` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `OK` | `OK` | `OK` | `NO` | `PARTIAL` | Keep separate, but align around workspaces/access groups/workflows. | P5 | `IN PROGRESS` | `6.3`, `6.8` | Bundle C - Attribution and Allocation. Rules and reports are shipped, and the active dimension list no longer advertises legacy `team`. Rule management still lacks update/edit and deeper cohesion with access groups/workflow attribution. |
| Ledger | `/ledger` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `OK` | `OK` | `OK` | `NO` | `PARTIAL` | Collapse into `Platform settings -> Compliance`; no separate route needed. | P5 | `NOT STARTED` | `6.10` | Bundle D - Compliance Closure. Snapshot generation and verification exist, but `/ledger` itself just redirects to `/settings?tab=compliance`; the feature is platform-admin compliance tooling, not a cohesive FinOps page. |
...
| Row Major Feature | Row Subfeature | Budgets | Budget detail | Budget overrides | Budget notifications | Billing periods | Billing period detail | Chargeback | Ledger | Finding |
|-------------------|----------------|---------|---------------|------------------|----------------------|-----------------|-----------------------|------------|--------|---------|
| Gateway & Routing | Provider profiles | `GAP` | `GAP` | `GAP` | `N/A` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `N/A` | Provider cost posture should become a much stronger bridge between Gateway and FinOps. |
| Gateway & Routing | Model gateway | `PARTIAL` | `GAP` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `N/A` | Gateway and FinOps already meet on runtime enforcement, but the operator story is still split. |
