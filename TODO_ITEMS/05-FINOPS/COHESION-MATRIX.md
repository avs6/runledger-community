...
Last updated: 2026-08-31 (05-FINOPS WU-004)
...
|-------------------|----------------|----------------------|--------------|------------|-------|------------|---------------|----------|--------------|-----------|--------------|--------|----------|-------------|---------|
| FinOps | Budgets | `STRONG` | `N/A` | `STRONG` | `PARTIAL` | `STRONG` | `STRONG` | `STRONG` | `N/A` | `STRONG` | `N/A` | `STRONG` | `N/A` | `N/A` | WU-001: Access groups and API keys are now first-class budget scopes with entity selector dropdowns in CreateBudgetModal. Org posture analytics endpoint provides workspace-level context. Telemetry downstream posture returns budget_count. Users page cross-links to Budgets. AI hub drills into model-scoped budgets via cost posture endpoint. |
| FinOps | Budget detail | `STRONG` | `N/A` | `PARTIAL` | `PARTIAL` | `STRONG` | `STRONG` | `STRONG` | `N/A` | `STRONG` | `N/A` | `STRONG` | `N/A` | `N/A` | WU-001: Budget detail now shows Org & Access Scope Context card with workspace user/key/group counts, 30d spend, scope entity attributes, and AI Hub model catalog usage. Drill-through links to Organization, Access Groups, API Keys, AI Hub, Telemetry, and Users. |
...
| FinOps | Budget overrides | `PARTIAL` | `STRONG` | `STRONG` | `STRONG` | `N/A` | `N/A` | `STRONG` | `N/A` | `STRONG` | `PARTIAL` | `STRONG` | `STRONG` | `PARTIAL` | `N/A` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `STRONG` | WU-002 (03-OBSERVE) adds explicit Budget Overrides drill-through links to Runs list, Run detail, Request flow, and Request explorer FinOps posture cards. Override counts visible on Analytics overview, investigation surfaces, Analytics Economics, Cost & Savings, and Monitoring via posture endpoints. |
...
| FinOps | Billing period detail | `PARTIAL` | `STRONG` | `STRONG` | `STRONG` | `PARTIAL` | `PARTIAL` | `STRONG` | `N/A` | `STRONG` | `STRONG` | `STRONG` | `STRONG` | `PARTIAL` | `STRONG` | `PARTIAL` | `PARTIAL` | `PARTIAL` | `STRONG` | WU-002 (03-OBSERVE) adds explicit Billing Period Detail drill-through links to Runs list, Run detail, Request flow, and Request explorer FinOps posture cards. Billing period detail reachable from Analytics overview, Model usage, Economics, Cost & Savings, Outcomes, and Monitoring through FinOps posture drill-through links. |
...
| FinOps | Chargeback | `PARTIAL` | `N/A` | `STRONG` | `N/A` | `N/A` | `N/A` | `N/A` | `STRONG` | `STRONG` | `STRONG` | `STRONG` | `STRONG` | 04-SAFETY WU-017 adds Runtime Scope & Evidence posture card (cyan theme) to Alert Rules via `GET /analytics/alert-rules-runtime-posture` with FinOps context (chargeback rules, active budgets, budget notifications 30d). Alert rules N/A→S. Prior: WU-016 Security N/A→S, WU-010 Audit log/Governance pack P→S, WU-003 Tags P→S. |
| FinOps | Ledger | `N/A` | `N/A` | `N/A` | `STRONG` | `N/A` | `N/A` | `STRONG` | `STRONG` | `N/A` | `STRONG` | `STRONG` | `PARTIAL` | 04-SAFETY WU-016 adds Runtime Scope & Evidence posture card (cyan theme) to Security via `GET /analytics/security-runtime-posture` with FinOps context (chargeback rules, ledger snapshots, ledger entries 30d). Security P→S. WU-015 adds Runtime Scope & Evidence posture card to Data Capture via `GET /analytics/data-capture-runtime-posture` with ledger context (snapshots, entries 30d). Data capture N/A→S. Prior: WU-013 Tool policies N/A→S, WU-010 Audit log P→S. |
