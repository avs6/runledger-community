...
`run_demo.py full-simulate` defaults to local-only Ollama scenarios and a 3x
traffic multiplier:
...
It imports `scripts/configs/pricing.yaml`, which is intentionally Ollama-only, so the demo
...
|---|---|---|
| `admin@runledger.local` | `runledger` | Platform admin created by the simulator. |
...
This is the primary automated demo profile. The manual labs remain separate on
purpose, and the quick REST-only seed is available from `run_demo.py seed-demo`
when you want the canonical HomeLab + LocalAIAgentStack foundation quickly.
...
python scripts\run_demo.py backup
python scripts\run_demo.py traffic --workspace "Python Console" --source python-console
...
Create a file in any folder, usually under `scripts/core/scenarios/ollama/`:
...
- [`labs/billing-reconciliation.md`](../../labs/billing-reconciliation.md)
  Period creation, reconciliation review, adjustment lifecycle, shared-cost policy management, and signed export verification.
- [`labs/chargeback.md`](../../labs/chargeback.md)
  Chargeback rule CRUD, report review, workflow-tag allocation, and finance-ready CSV/JSON exports.
- [`labs/ledger-closure.md`](../../labs/ledger-closure.md)
  Compliance closure readiness, ledger verification, and the evidence chain across billing, chargeback, backup, and audit.
