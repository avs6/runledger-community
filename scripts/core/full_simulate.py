...
platform admin and runs every scenario under ``scripts/core/scenarios/`` — each creates its
...
def _import_pricing(sim: Sim) -> None:
    """Upload configs/pricing.yaml to the provider-pricing catalog (best-effort)."""
...
def main() -> None:
    ...
    # 4. Governance & finops seeding (cross-workspace).
    say("\n→ seeding governance & finops features", "b")
    for ws in sim.workspaces:
        if not ws.key:
            continue
        ws.get_governance_audit_pack()

    # 5. Guardrails seeding (cross-workspace baseline content filters).
    say("\n→ seeding guardrails across all workspaces", "b")
    baseline_filters = [
    ...
    # 6. ML intelligence seeding (generate forecasts for each workspace).
    say("\n→ seeding ML intelligence (forecasts, top-K, patterns)", "b")
    for ws in sim.workspaces:
        if not ws.key:
            continue
        try:
