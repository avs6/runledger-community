...
4. **Run every scenario** — discovers every scenario under [`scenarios/`](./scenarios) (recursively, across folders) and runs each. Each scenario creates its own org + workspace and fills it via the API: gateway routes, runs (`/ingest/v1/batch`), budgets, outcomes, scores, and alerts.
