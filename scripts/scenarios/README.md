...
| `hosted/02_ml_research` | Nova Labs / Research | o3-mini, claude-sonnet-4-6, gpt-4o, llama3.1:8b, llama3.2 | Frontier-vs-local cost/quality; compiler + compression + tool filtering; intelligent routing; dense multi-metric eval scoring; benchmark-passed outcomes |
...
| `ollama/01_coding_assistant` | Acme Dev Tools / Coding Assistant | qwen2.5-coder:14b, deepseek-r1:14b/8b | **Priced local inference**; semantic cache; compiler + tool filtering + skill injection; intelligent routing; bug-resolved outcomes; GPU-spend budgets + alert |
...
> The simulator keeps two credentials per workspace: a dashboard session key for management
> actions (Gateway routes, Alert Rules, Budgets, API-key minting) and a workspace API key for
> agent/data-plane traffic. That mirrors the RBAC model used by the dashboard.
...
| `add_route(alias, model, priority=, **flags)` | A gateway route (`semantic_cache_enabled=`, `context_compiler_config=`, `intelligent_routing_enabled=`, `routing_config=`, `base_url=` for Ollama, etc.) |
| `add_budget(scope_type, limit_usd, period_type=, action=, scope_id=)` | A budget, created with the org-admin dashboard session for that workspace |
