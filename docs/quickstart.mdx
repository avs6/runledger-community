...
That command now launches the **core** profile. For the full local demo footprint:
...
| Qdrant | 8202 | Vector store (semantic cache, `aux` / `full-demo`) |
| Embedding svc | 8204 | fastembed (CPU, `aux` / `full-demo`) |
| Semantic cache | 8205 | Near-duplicate cache (`aux` / `full-demo`) |
| MCP gateway | 8206 | Claude Desktop / Code (`aux` / `full-demo`) |
| PostgreSQL | 5432 | — |
| Redis | 6379 | — |
| OTel Collector | 4317 / 4318 | OTLP/gRPC + HTTP (`observability` / `full-demo`) |
...
```bash
# Create a route on the control plane
...
<AccordionGroup>
  ...
  <Accordion title="Ollama / local provider shows $0 cost">
    Correct — Ollama has no API cost. Runs appear in analytics with `cost_usd = 0.00`. To assign an internal transfer price, add a pricing row via **Provider Profiles** or `POST /providers/pricing` for `provider = ollama`.
  </Accordion>
  <Accordion title="OTel Collector not forwarding traces">
    ...
    ```bash
    docker compose restart runledger-otel-collector
    docker compose logs runledger-otel-collector
    ```
    If you start the collector from scratch, prefer the dedicated profile:
    ```bash
