...
Edit `infra/.env` and set `SECRET_KEY`:
...
| API + Swagger | 8000 | FastAPI |
| API Reference | 8000/reference | Scalar UI |
| PostgreSQL | 5432 | — |
| Redis | 6379 | — |
...
<AccordionGroup>
  ...
  <Accordion title="OTel Collector not forwarding traces">
    Set `RUNLEDGER_API_KEY` in `infra/.env`. Without it, the Collector starts but gets 401s from the API. Restart after setting:
    ```bash
