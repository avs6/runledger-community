...
It runs as an inline gateway stage after the semantic cache and Context Compiler. **Fail-open**: if the
router is unavailable, the request routes to the alias it was sent to (or a configured default tier).
...
| Service | Port | Role |
|---|---|---|
| `runledger-router` | 8210 | Classify + tier mapping |
