---
title: "Runtime Controls"
description: "Per-route guardrails enforced in-band — cost caps, per-user rate limits, and PII redaction — so a runaway loop is stopped before it reaches the provider."
---

Because inference flows *through* the gateway, RunLedger can enforce controls **in the request path** —
a runaway agent is throttled or blocked before it ever reaches (and bills) the provider.

## Cost caps

Set a daily and/or monthly USD ceiling per route. When the route's spend crosses the cap, further requests
are rejected until the window resets.

```json
{
  "daily_cost_limit_usd": 50.00,
  "monthly_cost_limit_usd": 1000.00
}
```

## Per-user rate limits

Limit requests-per-minute per end user on a route. Pass the user via the
`X-RunLedger-End-User-Id` header; the limit is enforced in Redis on the hot path.

```json
{ "per_user_rpm_limit": 60 }
```

## PII redaction

When enabled on a route, message content is scrubbed of common PII patterns **before** it is forwarded to
the provider — useful for third-party models.

```json
{ "pii_redaction_enabled": true }
```

## Health auto-disable

A route that fails health checks repeatedly is automatically taken out of rotation (`health_auto_disable`),
so fallback routes absorb the traffic until it recovers.

## How it fits

```mermaid
flowchart LR
  req[Request] --> caps{Cost cap OK?}
  caps -- over --> block[Reject]
  caps -- ok --> rpm{Under RPM?}
  rpm -- over --> block
  rpm -- ok --> redact[PII redaction] --> fwd[Forward to provider]
```

All controls are set per route on the **Gateway** dashboard page or via `POST/PUT /gateway/routes`.

## FinOps budget bridge

Rate-limited routes contribute to throttle containment metrics that feed into budget
and billing analytics. Use the performance posture endpoints to see how rate limits
affect spend:

```bash
# Per-budget throttle containment
GET /analytics/budget-performance-posture/{budget_id}

# Workspace-wide rate-limit + billing view
GET /analytics/billing-period-performance-posture
```

The response includes `rate_limits.containment_coverage_pct` — the proportion of
active routes that have rate-limit guardrails configured.

### Guardrails observe posture

`GET /analytics/guardrails-observe-posture` returns a 30-day enforcement snapshot for
the workspace: rule coverage (total, active), evaluation breakdown (blocks, modifications,
allows, block/modification rates), mode breakdown (pre-call, post-call), false-positive
feedback count, and latency impact (avg, max). The guardrails page shows this as a
**Guardrails Observe Posture** card with drill-through links to Runs, Request Flow,
Request Explorer, Outcomes & ROI, Engineering, and Monitoring.

## Related

<CardGroup cols={2}>
  <Card title="Budgets" icon="shield-check" href="/finops/budgets">
    Workspace / user / feature spend limits with automatic actions.
  </Card>
  <Card title="Billing Periods" icon="calendar" href="/finops/billing">
    Period-level throttle economics and cost attribution.
  </Card>
  <Card title="Data retention" icon="clock-rotate-left" href="/governance/retention">
    Control how long payloads and telemetry are kept.
  </Card>
  <Card title="Runs" icon="play" href="/observe/runs">
    Guardrail enforcement outcomes visible in run investigation.
  </Card>
  <Card title="Monitoring" icon="monitor-waveform" href="/observe/monitoring">
    Guardrail firing rates and latency impact in live dashboards.
  </Card>
</CardGroup>
