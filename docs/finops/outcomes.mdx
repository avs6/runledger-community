...
```python
rl.outcome(run_id, outcome_type="refund_resolved", success=True, value_usd=12.00)
```

Or `POST /outcomes` with the run id. Outcomes roll up nightly into per-workflow metrics.
