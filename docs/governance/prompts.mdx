...
```python
prompt = rl.prompts.get("support-reply", variables={"tone": "friendly"})
resp = client.chat.completions.create(model="gpt-4o-mini", messages=prompt.messages)
```

The app always fetches the **production** version by name; promoting a new version rolls everyone forward
