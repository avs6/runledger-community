...
| Mode | Stores |
|---|---|
| `off` (default) | No payloads — metadata, tokens, and cost only. |
| `errors` | Payloads only for failed runs. |
| `sampled` | A configurable fraction of payloads. |
| `full` | All payloads. |

Set it via `RUNLEDGER_CAPTURE` (SDK) or per workspace. The [gateway PII redaction](/gateway/runtime-controls)
can additionally scrub content before it ever reaches a provider.
